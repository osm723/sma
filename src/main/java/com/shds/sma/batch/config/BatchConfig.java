...
import com.shds.sma.batch.processor.OrderItemProcessor;
import com.shds.sma.batch.processor.PersonItemProcessor;
...
public class BatchConfig {
     ...
     */
    @Bean
    public JpaPagingItemReader<Person> personReader() {
    ...
    // Step : reader, processor, writer를 이용한 청크 기반 처리 (청크 사이즈 10)
     ...
     */
    @Bean
    public Job importPersonJob(JobListener listener) {
    ...
    public JpaPagingItemReader<OrderEntity> orderReader() {
        ...
        reader.setEntityManagerFactory(emf);
        reader.setQueryString("SELECT o FROM OrderEntity o");
        reader.setPageSize(10);
        return reader;
    }
    ...
    @Bean
    public JpaItemWriter<OrderEntity> orderWriter() {
        JpaItemWriter<OrderEntity> writer = new JpaItemWriter<>();
        writer.setEntityManagerFactory(emf);
        return writer;
    }

    @Bean
    public Job processOrderJob(JobListener listener) {
    ...
    public Step orderStep() {
        return new StepBuilder("orderStep", jobRepository)
                .<OrderEntity, OrderEntity>chunk(10, platformTransactionManager)
                .reader(orderReader())
