...
public class BatchLogConfig {
    ...
    private static final LocalDateTime LOCAL_DATE_TIME_MINUS_ONE_DAY = LocalDateTime.now().minusDays(1);
    private static final LocalDateTime LOCAL_DATE_TIME_MINUS_TEN_MINUTE = LocalDateTime.now().minusMinutes(10);

    // ===== Job : errorLogDailyJob 데이터 처리 =====
    /**
     * Reader
     ...
     */
    @Bean
    public RepositoryItemReader<Log> errorLogDailyReader(LogRepository logRepository) {
        RepositoryItemReader<Log> reader = new RepositoryItemReader<>();
        reader.setRepository(logRepository);
        reader.setMethodName("findByLogDateAfter");
        reader.setArguments(Collections.singletonList(LOCAL_DATE_TIME_MINUS_ONE_DAY));
     ...
     */
    @Bean
    public ErrorLogDailyWriter errorLogDailyWriter() {
        return new ErrorLogDailyWriter(alarmService);
    }
     ...
     */
    @Bean
    public Job errorLogDailyJob(JobListener listener) {
        return new JobBuilder("errorLogDailyJob", jobRepository)
                .incrementer(new RunIdIncrementer())
                .listener(listener)
                .flow(errorLogDailyStep())
                .end()
                .build();
    }
     ...
     */
    @Bean
    public RepositoryItemReader<Log> errorLogMinuteReader(LogRepository logRepository) {
        RepositoryItemReader<Log> reader = new RepositoryItemReader<>();
        reader.setRepository(logRepository);
        reader.setMethodName("findByLogDateAfter");
        reader.setArguments(Collections.singletonList(LOCAL_DATE_TIME_MINUS_TEN_MINUTE));
        Map<String, Sort.Direction> sorts = new HashMap<>();
        sorts.put("id", Sort.Direction.ASC);
        reader.setSort(sorts);
        reader.setPageSize(100);
     ...
     */
    @Bean
    public ErrorLogMinuteWriter errorLogMinuteWriter() {
        return new ErrorLogMinuteWriter(alarmService);
    }
     ...
     */
    @Bean
    public Job errorLogMinuteJob(JobListener listener) {
        return new JobBuilder("errorLogMinuteJob", jobRepository)
                .incrementer(new RunIdIncrementer())
                .listener(listener)
                .flow(errorLogMinuteStep())
                .end()
                .build();
    }
